...
class RobustBackgroundSalience:
    """
    ...
    def _make_graph(self, grid):
        ...
        # undo hashing
        edges = [
            [vertices[x % num_vertices], vertices[x // num_vertices]] for x in edges
        ]
    ...
    def compute_saliency_map(self, obs: dict) -> np.ndarray:
        """
        Compute RBD saliency map from observation dictionary.
        ...
        sal = self._compute_saliency(img)

        # Normalize to [0, 1] to match protocol expectations
        sal_max = np.max(sal)
        if sal_max > 0:
            sal = sal.astype(np.float32) / sal_max
        else:
            sal = sal.astype(np.float32)

        return sal

    def _compute_saliency(self, img):
        """
        ...
        for v in vertices:
            ...
            if (
                np.any(x_pix == 0)
                or np.any(y_pix == 0)
                or np.any(x_pix == nrows - 1)
                or np.any(y_pix == ncols - 1)
            ):
                boundary[v] = 1
            else:
                boundary[v] = 0
        ...
        for v1 in vertices:
            for v2 in vertices:
                ...
                if v1 == v2:
                    ...
                    geodesic[i1, i2] = self._path_length(
                        all_shortest_paths_color[v1][v2], G
                    )
                    spatial[i1, i2] = (
                        scipy.spatial.distance.euclidean(centers[v1], centers[v2])
                        / max_dist
                    )
                    smoothness[i1, i2] = (
                        math.exp(-(geodesic[i1, i2] ** 2) / (2.0 * self.sigma_clr**2))
                        + self.mu
                    )
